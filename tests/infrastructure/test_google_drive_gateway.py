...
def fake_service(mocker):
    ...
    mocker.patch("infrastructure.gdrive.google_drive_gateway.MediaIoBaseDownload",
...
def test_list_files(fake_service):
    fake_service.files().list().execute.return_value = {
        "files": [
            ...
            {"id": "2", "name": "doc.xlsx", "mimeType": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "size": "2048"}
        ]
    }
